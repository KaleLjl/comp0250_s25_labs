...
public:
  ...
  */
  ...
  /** \brief Function to identify the shape of an object using
    ...
    */
  std::string
  check_shape(geometry_msgs::Point point);
  ...
  /** \brief Function to apply a filter to the point cloud.
    ...
    */
  void 
  apply_filter(PointCPtr &in_cloud_ptr,
  ...
  cluster_point_clouds(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud);
  ...
  /** \brief Function to get the width of a cluster.
    ...
    */
  double 
  get_cluster_width(pcl::PointCloud<pcl::PointXYZ>::Ptr cluster);
